...
export class DiscordBot {
    ...
    private async handleFeedbackInteraction(
        ...
        try {
            ...
            if (resolver) {
                ...
                if (embed) {
                    ...
                    if (row && 'components' in row) {
                        const updatedButtons = row.components.map(
                            (button: any) => {
                                const isSelected = button.customId.includes(
                                    `:${value}:`
                                );
            ...
            console.error(
                '[ERROR] Error handling feedback interaction:',
                error
            );
            await interaction
                .reply({
    ...
    async getThreadMessages(
        ...
        try {
            ...
            const threadMessages: import('../types/mcp').ThreadMessage[] =
                Array.from((messages as any).values()).map((message: any) => {
                    ...
                    if (includeEmbeds && message.embeds.length > 0) {
                        threadMessage.embeds = message.embeds.map(
                            (embed: any) => ({
