...
describe('DiscordBot', () => {
    ...
    beforeEach(() => {
        ...
        mockClient = {
            ...
            on: mock((_event: string, _handler: Function) => mockClient),
            once: mock((_event: string, _handler: Function) => mockClient),
    ...
    describe('テキスト応答時のEmbed更新', () => {
        ...
        it('テキスト応答待機時にmessageIdが保存される', async () => {
            ...
            expect(resolvers.get('test-thread-id').messageId).toBe(
                'test-message-id'
            );
