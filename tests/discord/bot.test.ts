...
import { Client, Events, type MessageCreateOptions, type Message } from "discord.js";
    ...
    describe("テキスト応答時のEmbed更新", () => {
        ...
        beforeEach(() => {
            ...
            mockClient.on = mock((event: string, handler: Function) => {
                ...
                // 他のイベントもハンドルする
                originalOn.call(mockClient, event, handler);
                return mockClient;
            });
        ...
        it("テキスト応答待機時にmessageIdが保存される", async () => {
            ...
            resolvers.get("test-thread-id").resolve({ message: "test", userId: "test-user" });
            await promise;
        });
        ...
        it("メッセージ取得に失敗してもエラーハンドリングされる", async () => {
            ...
            // メッセージ取得を失敗させる
            mockThread.messages.fetch = mock(() => Promise.reject(new Error("Message not found")));
